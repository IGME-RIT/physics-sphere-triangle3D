...
//The basic structure required for a triangle. we only need 3 points, wbut we use the origin to signify 
//the model Coorniate origin in world space i.e. it is used to move the three points collectively.
struct Triangle{
	glm::mat4 MVP;
	glm::vec3 origin;
	glm::vec3 point1;
...
{
	//First convert the points to world space from model space
	glm::vec4 p1, p2, p3;
	// Multiply it with rotation and translation matrices
	...
	//if the distance between the closest point and the center of the sphere is larger than the raiud, then no collision else, collision is detected. 
	if (glm::distance(s.origin, ClosestPointTriangle(s.origin, a, b, c)) <= s.radius)
		return true;
	else
		return false;
}
...
void update()
{
	// if the objects are colliding then change color, else revert back to original color
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	////This set of controls are used to move the triangle in z axis
	if (key == GLFW_KEY_W && action == GLFW_PRESS)
		triangle.origin.z -= moverate;
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "Sphere - Triangle Collision detection", nullptr, nullptr);

	std::cout << "\n This is a collision test between a sphere and a 2D triangle in 3D.\n\n\n\n\n";
	std::cout << "Use Mouse to move in x-y plane, and \"w and s\" to move in z axis. \n";
	std::cout << "Click and drag the left mouse butoon to torate the triangle.";
